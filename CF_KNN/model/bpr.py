...
class BPR(CF_KNN):
    ...
    def fit(self, verbose=True):
        if self.hparam['domain'] == 'course':
            if self.use_gpu:
                self.model_course = BayesianPersonalizedRanking(
                    factors=self.hparam['bpr_course_factors'],
                    learning_rate=self.hparam['bpr_course_lr'],
                    iterations=self.hparam['bpr_course_iterations'],
                    random_state=self.hparam['random_state']
                )
            else:
                self.model_course = implicit.cpu.bpr.BayesianPersonalizedRanking(
                    factors=self.hparam['bpr_course_factors'],
                    learning_rate=self.hparam['bpr_course_lr'],
                    iterations=self.hparam['bpr_course_iterations'],
                    random_state=self.hparam['random_state']
            ...
            print('fit BPR for course prediction...')
            ...
            if self.use_gpu:
                self.model_topic = BayesianPersonalizedRanking(
                    factors=self.hparam['bpr_topic_factors'],
                    learning_rate=self.hparam['bpr_topic_lr'],
                    iterations=self.hparam['bpr_topic_iterations'],
                    random_state=self.hparam['random_state']
                )
            else:
                self.model_topic= implicit.cpu.bpr.BayesianPersonalizedRanking(
                    factors=self.hparam['bpr_topic_factors'],
                    learning_rate=self.hparam['bpr_topic_lr'],
                    iterations=self.hparam['bpr_topic_iterations'],
                    random_state=self.hparam['random_state']
            ...
            print('fit BPR for topic prediction...')
    ...
    def pred_course(self, output=True):
        ...
        if output:
            ...
            predict(
                ...
                path=f'{output_dir}/bpr.csv',
                user_id=[i['user_id'] for i in test_data],
                domain='course'
            )
        else:
            self.pred_course_test = pred
        print('complete the process of BPR course prediction!')
    ...
    def pred_topic_from_course(self, output=True):
        ...
        if output:
            ...
            predict(
                result=pred_topic, 
                path=f'{output_dir}/bpr_from_course.csv',
                user_id=[i['user_id'] for i in test_data],
                domain='topic'
            )
        else:
            self.pred_topic_test = pred_topic
        print('complete the process of BPR topic prediction(from course)!')
    ...
    def pred_topic(self, output=True):
        ...
        print('complete the process of BPR topic prediction!')